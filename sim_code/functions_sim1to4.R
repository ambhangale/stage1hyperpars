...
# MCSampID = 1; n = 5; G = 3
# rr.data <- genGroups(MCSampID = 1, n = 5, G = 3, smallvar = T)
# rr.vars <- c("V1", "V2", "V3")
...
getSigma <- function(return_mats = TRUE, smallvar = FALSE){
  ...
  if (smallvar) {
    ...
    SIGMA_c["f3@P", "f2@A"] <- SIGMA_c["f2@A", "f3@P"] <- 
      R_c["f3@P", "f2@A"] * sqrt(SIGMA_c["f3@P", "f3@P"]) * sqrt(SIGMA_c["f2@A", "f2@A"])
    SIGMA_c["f3@P", "f3@A"] <- SIGMA_c["f3@A", "f3@P"] <- 
      R_c["f3@P", "f3@A"] * sqrt(SIGMA_c["f3@P", "f3@P"]) * sqrt(SIGMA_c["f3@A", "f3@A"])
    SIGMA_c["f3@P", "f1@P"] <- SIGMA_c["f1@P", "f3@P"] <- 
      R_c["f3@P", "f1@P"] * sqrt(SIGMA_c["f3@P", "f3@P"]) * sqrt(SIGMA_c["f1@P", "f1@P"])
...
# getSigma(return_mats = FALSE, smallvar = TRUE)
# getSigma(return_mats = TRUE)
# getSigma(return_mats = TRUE, smallvar = TRUE)

#----

# function 1: simulate data for a single group----

genData <- function(n, smallvar = FALSE) {
  
  SIGMA_mats <- getSigma(smallvar = smallvar)
...
# genData(n = 5)
# genData(n = 5, smallvar = T)

#----

# function 2: simulate data for multiple groups----

genGroups <- function(MCSampID, n, G, smallvar = FALSE) {
  ...
  gDat <- lapply(1:G, function(g){
    ...
    gen <- genData(n = n, smallvar = smallvar)
...
# genGroups(MCSampID = 1, n = 5, G = 3, smallvar = TRUE)
...
#                  default_prior = lavaan.srm::srm_priors(data = rr.data[c("V1", "V2", "V3")]))
# prophetic_priors(pop_corMat = list(popCorr_c = getSigma(smallvar = TRUE)$R_c,
#                                    popCorr_d = getSigma(smallvar = TRUE)$R_d),
#                  pop_SDvec = list(popSD_c = sqrt(diag(getSigma(smallvar = TRUE)$SIGMA_c)),
#                                   popSD_d = sqrt(diag(getSigma(smallvar = TRUE)$SIGMA_d))),
#                  precision = 0.1,
#                  default_prior = lavaan.srm::srm_priors(data = rr.data[c("V1", "V2", "V3")])) # smallvar condition
...
set_priors <- function(data, rr.vars, IDout, IDin, IDgroup, priorType, targetCorr,
  ...
  prior_env$pop_corMat <- list(popCorr_c = getSigma(smallvar = smallvar)$R_c,
                               popCorr_d = getSigma(smallvar = smallvar)$R_d)
  prior_env$pop_SDvec <- list(popSD_c = sqrt(diag(getSigma(smallvar = smallvar)$SIGMA_c)),
                              popSD_d = sqrt(diag(getSigma(smallvar = smallvar)$SIGMA_d)))
...
ogsat <- function(MCSampID, n, G, smallvar = FALSE, savefile = FALSE) {
  ...
  dat <- genGroups(MCSampID = MCSampID, n = n, G = G, smallvar = smallvar)
...
# makeShell(analType = "prophetic", precision = 0.05, sim = "sim1", wallTime = "10-23:59:59")
# makeShell(analType = "prophetic", precision = 0.1, sim = "sim1", wallTime = "10-23:59:59")
# makeShell(analType = "prophetic", precision = 0.2, sim = "sim1", wallTime = "10-23:59:59")
# makeShell(analType = "FIML1S", sim = "sim1", wallTime = "10-23:59:59")
# makeShell(analType = "thoughtful", precision = 0.1, sim = "sim2", wallTime = "10-23:59:59")
# makeShell(analType = "ANOVA", precision = 0.1, sim = "sim2", wallTime = "10-23:59:59")
# makeShell(analType = "FIML", precision = 0.1, sim = "sim2", wallTime = "10-23:59:59")
